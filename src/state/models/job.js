...
function formatJob(values) {
	...
	return {
		...
		team: team,
		recurrence: details.recurringType || '',
	};
}
...
function formatGoogleCalendarEvent(values) {
	...
	return {
		...
		description: `Customer Name: ${customer.name}, CustomerId: ${customer.docId}, Team Name: ${team.name}, Team Id: ${team.docId}, zipcode: ${address.zipcode}, type: ${details.cleaningType}, recurrence: ${details.recurringType}`,
		extendedProperties: {
			shared: {
		...
		if (calEvent.extendedProperties) {
			...
			return {
				...
				details: {
					...
					recurrence: shared.recurringType || '',
				},
			};
		} else {
